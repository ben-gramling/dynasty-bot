...
- **Target return (v3.3.1, the user's RANGE):** define `return(pair) = combined ΔW(me) ÷ Σ v(assets I send across both legs)` — ledger gain on face-value inventory deployed. v3.4: the numerator is the COMBINED ledger delta of §2 (both legs applied together — not the sum of leg ΔWs, which no longer adds), the denominator stays face KTC (what the market prices). The per-leg return floor is retired: a pair's buy leg is legitimately negative in isolation; legs are pre-ranked inside the pool by isolation ΔW as a disclosed heuristic, and every STORED pair's return is exact combined. The UI supplies a range `[R_min, R_max)` over the presets (min 1 / 2.5 / 5 / 10 / 20%, default 5; max 2.5 / 5 / 10 / 20% or no cap, default no cap; min < max enforced); the board lists the stored legal pairs inside the range, ranked by return desc within the selected range. Storage is **stratified per band**: the nightly run keeps the top `pairs_per_band` (100) pairs per return band — bands derived from the presets as half-open intervals [1,2.5), [2.5,5), [5,10), [10,20), [20,∞) — sorted return-desc within band, bands concatenated descending (so the stored list also reads return-desc globally), with per-band honest counts (`bands`: {lo, hi, stored, count, saturated} — a saturated count is a verified floor, never an estimate; v3.3's top-500-overall storage put every stored pair at the top of the space and could not serve a range query). **v3.4 makes every band count a floor**: the walks cross legs ordered by their ISOLATION ΔWs (the disclosed pre-ranking heuristic) while each kept pair is priced by its EXACT combined ledger, and the two orderings disagree wherever the legs interact — so no cutoff can certify that the space above it was fully enumerated. Only a walk over the entire crossing space can, and it fires only in markets sparse enough to afford it. `counts_by_threshold` and `truncated` remain for ≥-style reads; range filtering is instant client-side.
...
- **Ranking (v3.3):** pairs sort by `return_pct` descending within the user's target filter — posture fit is no longer a rank key because it is a hard pool constraint (§5 posture bullet); every leg individually passes §3. Pair cards carry an `overlaps` count (shared-asset conflicts against other stored pairs); unpaired-leg data keeps exact `exclusive_with`.
...
1. **Sell-leg, ledger yes / gate no (v3.4 flip):** Mike Evans (4,125) + Courtland Sutton (3,674) → jaketoppen for vishan's 2027 1st (7,398) + jaketoppen's 2028 4th (1,759). Ledger: `ΔW(me) = +9,093` (starters −64, picks +9,157 — both WRs sit just outside my max-Σv lineup, so shipping them costs almost no `S` and buys 9,157 of `P`); `ΔW(them) = −7,941` (starters +1,216, picks −9,157) — per side, no negation. Gate: KTC's calculator shows 7,799 against 12,339, a **4,540 gap = 36.8%** of the larger side against a 2,468 band ⇒ **REJECTED**. The concentrated 7,398 pick earns a top-up that two mid-tier WRs do not; under v3.3's fitted consolidation curve the same trade read 17.3% and passed. Anti-fleece is clean (1.17).
2. **Buy-leg, negative alone (v3.4 semantics):** buy Jauan Jennings (3,026) from millj for my 2028 3rd (2,468). Ledger: `ΔW(me) = −2,468` (starters 0, picks −2,468) — Jennings does not crack my max-Σv lineup, so the buy is pure pick spend, which is *normal* for a buy leg and is why the per-leg return floor was retired; the paired sell recoups. Gate: 2,468 against 3,898, a **1,430 gap = 36.7%** against a 780 band ⇒ **REJECTED** (ratio 1.22, clean). Shape is still right: picks → SELLER-classified counterparty.
3. **Gate rejection:** Cam Ward (4,426) for Shedeur Sanders (2,368): ratio 1.87 > 1.35 — never surfaced; fleeces don't clear.
4. **Both ledgers gain (the arbitrage, §2):** my Javonte Williams (5,460) ↔ ronakpatel32's Zay Flowers (5,651), no picks either way. `ΔW(me) = +191` (starters +191), `ΔW(them) = +96` (starters +96) — the sum is positive because each side's starting lineup improves; zero-sum would have made this impossible to see. Gate PASS. It is the fixture's only gate-clean 1-for-1 where both ledgers rise, which is exactly why §11.1 pins it.

## 11. Implementation invariants (the test suite)

1. **Face conservation:** on every leg, Σ face v leaving one side equals Σ face v arriving at the other, exactly (bookkeeping). `ΔW` itself is per-side (v3.4) — a pinned example shows a pair positive for BOTH ledgers.
2. **Independence regressions:** perturb every league-tab lineup parameter (`q_*`, `u_*`, replacement), every roster count, and add/remove any bench player who cannot crack the max-Σv lineup — no recommended pair's `ΔW` changes. Grep-level: the trade path imports only the raw starter-sum solve, never the q/u/replacement machinery.
