...
def test_gate_is_the_exact_ktc_calculator(league):
    ...
    # v7.6: the pick side enters at the flat Mid tranche — the slot is never
    # estimated, and that IS their price for every consumer
    direct = ka.ktc_adjustment(
        [4125.0, 3674.0], [6118.0, 1759.0], top_value=league.top_ktc_value
    )
...
def test_worked_example_1_sell_leg(league):
    ...
    v4 reports the two coordinates and no blend; **v7.6 prices both picks at
    the flat Mid tranche for EVERY consumer** (6,118 and 1,759 — the retired
    rank_L lens read vishan's 1st Early 7,398; v7.5's pessimism read it Late
    5,562; the midpoint splits the difference). My side is (ΔS −64, ΔF +78): a
    PREFERENCE trade — good only for δ beyond δ* ≈ 0.4507 — and no longer the
    guaranteed loss v7.5's Late pricing certified (−786) nor the +1,358 gift
    the Early projection once claimed. Their side is (+1,216, −78) — one
    symmetric price vector, so the two ΔFs negate exactly. THE GATE MOVED
    ...
    assert get == {"2027 R1 (from vishan)": 6118, "2028 R4 (own)": 1759}
    ...
    assert (g["gap"], g["gap_pct"], g["band"]) == (2463.0, 24.0, 2052.4)
    assert g["band_ok"] is False
    assert g["raw_ratio"] == 1.01 and g["ratio_ok"]
    assert g["verdict"] == "FAIL: outside fairness band (gap 24.0% > band)"
    # §4a v5 favor from the SAME adjusted totals: 100·(7,799−10,262)/18,061 =
    # −13.64 → −13.6 in the calculator's own 1-decimal quantization; negative
    # = skewed to ME (they'd want more, which is also why the band fails)
    assert card["favor"] == g["favor"] == -13.6
    ...
    assert card["anchor_ask"]["ask"] == round(1.08 * (6118 + 1759))
...
def test_worked_example_2_buy_leg_face_for_face(league):
    ...
    coordinate. **v7.6 prices the pick at the flat Mid tranche everywhere**
    ...
    # market skim off the same one price: (3,001 − 2,468) ÷ 2,468
    assert card["market_return_pct"] == 21.6
    g = card["gate"]
...
def test_pair_coords_math_pinned(league):
    ...
    assert sell["breakeven"]["me"] == round(-1312.0 / (-1312.0 - 1636.0), 4) == 0.445
...
def test_exhaustiveness_spot_check(league, pool):
    ...
    (−1P,+1pk) slots under v7.6 go to {2026 1.06, 2026 1.12} and
    {2026 1.06, 2027 R2 (from millj)} — the same two v7.4 kept, restored by
    the symmetric price after v7.5 briefly swapped the second for
    {2026 2.04, 2027 R1 (own)} — displacing {2026 1.12, 2028 R1 (own)}
    (dropped by v7.4, still out). All tie on isolation floor at Burrow's
    −1,312 slot dent, so the shortlist turns entirely on the ΔF tie-break —
    exactly the coordinate every re-pricing moves.
    ...
    own −13.51% because a pair's floor here is ΔS and its denominator the
    face SENT, neither of which depends on which get-package came back."""
...
def test_board_pairs_verdict_true_and_honest_on_fixture(board, params):
    """§5 v7.6 / §11.12(g) on the committed fixture. Storage is stratified by
    FAVOR bucket (pair favor = min(f_buy, f_sell)): 1,200 pairs survive as the
    ...
    ([0,+5) 0 → 282); v7's pessimistic ΔF lens contracted the board
    1,178 → 925 ([0,+5) → 89); v7.4's numbered current-year picks blew it
    open, 925 → 1,372 with [+5,∞) non-empty (184) for the first time; v7.5's
    everywhere-pessimism handed the frontier straight back (1,184,
    [0,+5) 284, [+5,∞) 0).

    **v7.6 (flat Mid) lands between its neighbours: 1,200 stored, [0,+5)
    SATURATED at 300 for the first time — and [+5,∞) still 0.** The
    symmetric price restores the counterparty-friendly window v7.5's
    Early-out pricing had squeezed, but the far bucket stays empty: v7.4's
    184 pairs there rode the rank_L asymmetries (Early-projected picks
    bought at their cheap tranche while the gate charged the counterparty's
    calculator more), and a symmetric vector offers no such wedge. Every
    count below is a verified floor over the same walks that filled the
    bucket in v7.4.

    The headline pair is v7.4's, RESTORED number for number (its only pick is
    the current-year 1.01, which every lens priced identically — see
    test_sound_bound.test_board_headline_pair_survives_v5_1 for the round
    trip). Its ΔF is FRACTIONAL (+4,439.1) because the 1.01 is
    7,994.860000000001 — KTC derives the top two slots off the rookie ladder
    unrounded, and we carry it rather than rounding the engine to make a pin
    tidy.
    ...
    # 2026 1.01, and sells Stefon Diggs to josbaski for his 2026 3.05.
    top = doc["pairs"][0]
    ...
    # v7.6 re-pin, 1,184 → 1,200 (1,372 in v7.4, 925 in v7).
    # Four of the five buckets are saturated at the 3-heap union bound.
    assert len(doc["pairs"]) == sum(b["stored"] for b in bands) == 1200
    # the counterparty-favorable end: [0,+5) — trades their OWN calculator
    # scores as even-or-better for them — 0 (v5.1) → 282 (v6) → 89 (v7) → 288
    # (v7.4) → 284 (v7.5) → 300 SATURATED (v7.6), and [+5,∞) — a clear win
    # for them — 0 → 184 (v7.4) → 0 (v7.5) → 0 (v7.6): the symmetric price
    # offers no wedge to populate it, measured and accepted.
    assert [b["stored"] for b in bands] == [300, 300, 300, 300, 0]
    for b in bands:
    ...
    t = doc["truncated"]
    assert t is not None and t["stored"] == 1200
    assert t["total"] >= 1200 and t["total_saturated"] is True
...
def test_return_bands_and_favor_bucket_math():
    ...
    assert tr.bucket_index(buckets, -10.4) == 0  # v7.6 fixture top-pair favor min
